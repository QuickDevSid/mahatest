...
<div class="modal fade" id="addquiz" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <form id="getquizDetails" enctype="multipart/form-data" action="<?php echo base_url()?>Gatavarshichya_prashna_patrika_practice_test/add_exam" method="POST" >
        ...
        if(id>0)
        {
            $.ajax({
                ...
                success:function(response){
                    ...
                    $("#a_prashn_patrika").append("<option value=''>Select Exam </option>");
                    for( var i = 0; i<len; i++)
                    {
        ...
        if(id>0)
        {
            $.ajax({
                ...
                success:function(response){
                    var len = response.length;
                    $("#a_year").empty();
                    $("#a_year").append("<option value=''>Select Subject</option>");
                    for( var i = 0; i<len; i++)
                    {
                        var id = response[i]['id'];
                        var name = response[i]['name'];
