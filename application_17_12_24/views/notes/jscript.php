...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#Notes .menu-toggle').addClass('toggled');
        $('#Notes .ml-menu').css('display','block');

        $('#Notes_Note').addClass('active');
...
//Table data featching.
        var ur = "<?php echo base_url() ?>Notes/fetch_data";
        //Exportable table
        $('#user_data').DataTable({
            dom: 'Bfrtip',
            destroy: true,
    ...
    function getNotesDetailsEdit(getID)
    {
        var lid = getID.replace('edit_', '');
        $.ajax({
            type: "GET",
            url: "<?php echo base_url() ?>Notes/notesById/" + lid, // replace 'PHP-FILE.php with your php file
            dataType: "json",
            success: function (result) {
                if(result["id"])
                {
                    $('#edit_id').val(result["id"]);
                    $('#edit_NotesTitle').val(result["title"]);

                    var eid = result["selected_exams_id"];
                    var res = eid.substring(2, 3);
                   
                    $('#edit_SelectedExamId').selectpicker('val', res);

                    $('#edit_NotesSubjectId').selectpicker('val', result["notes_subject_id"]);
                    $('#edit_NotesStatus').selectpicker('val', result["status"]);
    ...
    function deleteDetails(getID) {
        ...
        swal({
            ...
            deleteNotesDetailsPerform(lid);
        });
    }

    function deleteNotesDetailsPerform(getID) {
    ...
    function showConfirmMessage(getID) {
        ...
        if (lid === "") {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() ?>Notes/deleteNotes",
                data: "id=" + lid,
