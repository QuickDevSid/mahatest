...
class Exam_Material extends CI_Controller
{
    ...
    public function subject_list()
    {
        ...
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/subject_list_script.php', $data);
    }
    ...
    public function exam_list()
    {
        ...
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/examscript_list.php', $data);
    }
    ...
    public function exam_sub_list()
    {
        ...
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/exam_sub_list_script.php', $data);
    }
    ...
    public function exam_year_list()
    {
        ...
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/exam_year_script_list.php', $data);
    }
    ...
    public function add_examwise_test()
    {
        $validate = $this->input->post('validate');
        $this->form_validation->set_rules('validate', 'Previous Year Pdf', 'required');
        if ($this->form_validation->run() === FALSE) {
            $data['single'] = $this->Exam_Material_model->get_single_exam_year_test();
            $exam_material_id = $id = $this->uri->segment(3);
            $data['exam_material_id'] = $exam_material_id;
            $this->load->view('exam_material/add_examwise_test', $data);
            $this->load->view('templates/header1', $data);
            $this->load->view('templates/menu', $data);
            $this->load->view('templates/footer1', $data);
            $this->load->view('exam_material/previous_testscript.php', $data);
        } else {
            $res = $this->Exam_Material_model->set_examwise_tests_details();
            if ($res == "1") {
                // echo "1";
                // exit;
                $this->session->set_flashdata("success", "Exam Test details added successfully!");
                redirect('exam_material/examwise_test_list');
            } elseif ($res == "2") {
                $this->session->set_flashdata("success", "Exam Test entry updated!");
                // echo "2";
                // exit;
                redirect('exam_material/examwise_test_list');
            } else {
    ...
    public function examwise_test_list()
    {
        $data['category'] = $this->Exam_Material_model->get_examwise_test_list();
        // echo '<pre>';
        // print_r($data['category']);
        // exit();
        $this->load->view('exam_material/examwise_test_list', $data);
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/previous_testscript.php', $data);
    ...
    {
        $this->Exam_Material_model->delete_examwise_test($id);
        $this->session->set_flashdata('delete', 'Record deleted successfully');
        redirect('exam_material/examwise_test_list');
    }

    public function add_subjectwise_test()
    {
        $validate = $this->input->post('validate');
        $this->form_validation->set_rules('validate', 'Subjectwise Test', 'required');
        if ($this->form_validation->run() === FALSE) {
            $data['single'] = $this->Exam_Material_model->get_select_subject();
            $exam_material_id = $id = $this->uri->segment(3);
            $data['exam_material_id'] = $exam_material_id;
            $this->load->view('exam_material/add_subjectwise_test', $data);
            $this->load->view('templates/header1', $data);
            $this->load->view('templates/menu', $data);
            $this->load->view('templates/footer1', $data);
            $this->load->view('exam_material/subjectwise_testscript.php', $data);
        } else {
            $res = $this->Exam_Material_model->set_subjectwise_tests_details();
            if ($res == "1") {
                // echo "1";
                // exit;
                $this->session->set_flashdata("success", "Subject Test details added successfully!");
                redirect('exam_material/subjectwise_test_list');
            } elseif ($res == "2") {
                $this->session->set_flashdata("success", "Subject Test entry updated!");
                // echo "2";
                // exit;
                redirect('exam_material/subjectwise_test_list');
            } else {
    ...
    public function subjectwise_test_list()
    {
        $data['category'] = $this->Exam_Material_model->get_subjectwise_test_list();
        // echo '<pre>';
        // print_r($data['category']);
        // exit();
        $this->load->view('exam_material/subjectwise_test_list', $data);
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/previous_testscript.php', $data);
    ...
    {
        $this->Exam_Material_model->delete_subjectwise_test($id);
        $this->session->set_flashdata('delete', 'Record deleted successfully');
        redirect('exam_material/subjectwise_test_list');
    }

    public function add_subjectwise_pdf()
    {
        $validate = $this->input->post('validate');
        $this->form_validation->set_rules('validate', 'Subjectwise Pdf', 'required');
        if ($this->form_validation->run() === FALSE) {
            $data['single'] = $this->Exam_Material_model->get_single_subjectwise_pdf();
            $exam_material_id = $id = $this->uri->segment(3);
            $data['exam_material_id'] = $exam_material_id;
            $this->load->view('exam_material/add_subjectwise_pdf', $data);
            $this->load->view('templates/header1', $data);
            $this->load->view('templates/menu', $data);
            $this->load->view('templates/footer1', $data);
            $this->load->view('exam_material/subjectwise_pdfscript.php', $data);
        } else {
            $indices = $this->input->post('indices');
            $upload_image = $this->input->post('current_image');
            $upload_pdf = $this->input->post('current_pdf');
                ...
                for ($i = 0; $i < count($indices); $i++) {
                    ...
                    if (isset($_FILES['image_' . $indices[$i]]) && $_FILES['image_' . $indices[$i]]['name'] != "") {
                        ...
                        if ($this->upload->do_upload('image_' . $indices[$i])) {
                            ...
                            echo "problem in second image";
                            exit;
                            redirect('exam_material/subjectwise_pdf_list');
                            return;
                        }
                    }
                    if (isset($_FILES['pdf_' . $indices[$i]]) && $_FILES['pdf_' . $indices[$i]]['name'] != "") {
            ...
            $res = $this->Exam_Material_model->set_subjectwise_pdf_details($upload_image, $upload_image_update, $upload_pdf, $upload_pdf_update);
            if ($res == "1") {
                // echo "1";
                // exit;
                $this->session->set_flashdata("success", "Ebook Setup details added successfully!");
    ...
    public function subjectwise_pdf_list()
    {
        $data['category'] = $this->Exam_Material_model->get_subjectwise_pdf_list();
        // echo '<pre>';
        // print_r($data['category']);
        // exit();
        $this->load->view('exam_material/subjectwise_pdf_list', $data);
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('exam_material/previous_pdfscript.php', $data);
    ...
    {
        $this->Exam_Material_model->delete_subjectwise_pdf($id);
        $this->session->set_flashdata('delete', 'Record deleted successfully');
        redirect('exam_material/subjectwise_pdf_list');
    }
}
