...
class CurrentAffairs extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['category'] = $this->CurrentAffairs_model->get_current_affairs_category();
    ...
    public function add_current_affairs_category_form()
    {
        $this->load->model("CurrentAffairs_model");
        $data['title'] = ucfirst('All CurrentAffairs'); // Capitalize the first letter
        $data['single'] = $this->CurrentAffairs_model->get_single_current_affairs_category($this->uri->segment(2));
        // echo 'hiii'; print_r($data['single']);  exit;
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('current_affairs/add_current_affairs_category', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('current_affairs/jscript.php', $data);
    }
    public function current_affairs_category()
    {
        $this->load->model("CurrentAffairs_model");
        $data['title'] = ucfirst('All CurrentAffairs'); // Capitalize the first letter
        $data['category'] = $this->CurrentAffairs_model->get_current_affairs_category();
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('current_affairs/current_affairs_category', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('current_affairs/category_script.php', $data);
    }
    ...
    public function add_test()
    {
        $this->load->model("CurrentAffairs_model");
        $data['title'] = ucfirst('All CurrentAffairs'); // Capitalize the first letter
        $data['single'] = $this->CurrentAffairs_model->get_single_course_tests($this->uri->segment(3));
        $data['category'] = $this->CurrentAffairs_model->get_current_affairs_category();
        // echo 'hiii'; print_r($data['single']);  exit;
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('current_affairs/add_test', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('current_affairs/jscript.php', $data);
    }
    public function tests()
    {
        $this->load->model("CurrentAffairs_model");
        $data['title'] = ucfirst('All CurrentAffairs'); // Capitalize the first letter
        $data['tests'] = $this->CurrentAffairs_model->get_course_tests();
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('current_affairs/tests', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('current_affairs/jscript.php', $data);
    }
    ...
    public function add_course_tests()
    {
        if (isset($_POST)) {
            ...
            $this->load->model("CurrentAffairs_model");
            $insert = $this->CurrentAffairs_model->save_course_tests();
            redirect('current_affairs_quiz');
        }
    }
    public function delete_test()
    {
        $this->load->model("CurrentAffairs_model");
        $insert = $this->CurrentAffairs_model->delete_test($this->uri->segment(3));
        redirect('current_affairs_quiz');
    }
    public function add_current_affairs_category()
    {
        if (isset($_POST)) {
            ...
            $insert = $this->CurrentAffairs_model->add_current_affairs_category();
            redirect('current_affairs_category');
        }
    }
    public function delete_current_affairs_category()
    {
        $this->load->model("CurrentAffairs_model");
        $insert = $this->CurrentAffairs_model->delete_current_affairs_category($this->uri->segment(2));
        redirect('current_affairs_category');
    }
    ...
    public function addPost()
    {
        ...
        $created_on = $this->now();
        $data = array(
            "current_affair_title" => $PostTitle,
    ...
    // public function manage_current_affairs_form()
    // {
    ...
    //     $this->load->view('templates/header1', $data);
    //     $this->load->view('templates/menu', $data);
    //     $this->load->view('current_affairs/manage_current_affairs_form', $data);
    //     $this->load->view('templates/footer1', $data);
    //     $this->load->view('current_affairs/jscript.php', $data);
    ...
    {
        $this->CurrentAffairs_model->get_duplicate_current_affair_title();
    }
    ...
    public function manage_current_affairs_form()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            $data['single'] = $this->CurrentAffairs_model->get_single_current_affairs();
            $this->load->view('templates/header1', $data);
            $this->load->view('templates/menu', $data);
            $this->load->view('current_affairs/manage_current_affairs_form', $data);
            $this->load->view('templates/footer1', $data);
            $this->load->view('current_affairs/currentaffairjscript.php', $data);
            ...
            if (isset($_FILES['image']) && $_FILES['image']['name'] != "") {
                $gst_config = array(
                    'upload_path'   => "assets/uploads/current_affairs/images",
                    'allowed_types' => "*",
                    'encrypt_name'  => true,
                );
                ...
                if ($this->upload->do_upload('image')) {
                    ...
                    echo "new error";
                    exit;
                    redirect('current_affairs/manage_current_affairs_form_list');
                    return;
                }
            }

            $res = $this->CurrentAffairs_model->set_current_affairs_details($upload_image);
    ...
    public function manage_current_affairs_form_list()
    {
        $data['category'] = $this->CurrentAffairs_model->get_single_current_affairs_list();
        // echo '<pre>';
        ...
        $this->load->view('templates/menu', $data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('current_affairs/currentaffair_list_script.php', $data);
        // $this->load->view('courses/newjscript.php', $data);
    }

    public function manage_current_affairs_form_inactive($id)
    {
        $this->CurrentAffairs_model->manage_current_affairs_form_inactive($id);
        redirect('current_affairs/manage_current_affairs_form_list');
    }

    public function manage_current_affairs_form_active($id)
    {
        $this->CurrentAffairs_model->manage_current_affairs_form_active($id);
        redirect('current_affairs/manage_current_affairs_form_list');
    }

    public function delete_manage_current_affairs_form($id)
    {
        $this->CurrentAffairs_model->delete_manage_current_affairs_form($id);
