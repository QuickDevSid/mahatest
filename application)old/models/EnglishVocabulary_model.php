...
class EnglishVocabulary_model extends CI_Model
{
    ...
    public function set_english_vocabulary_cat_details()
    {
        ...
        $data = array(
            'vocabulary_category_name' => $this->input->post('title'),
        ...
        if ($id) {
            ...
            $this->db->where('id', $id);
            $this->db->update('tbl_vocabulary_category', $data);
            return 0;
        } else {
            // echo "into insert";
            // exit;
            $this->db->insert('tbl_vocabulary_category', $data);
    ...
    {
        $this->db->where('tbl_vocabulary_category.status', '1');
        $this->db->where('tbl_vocabulary_category.is_deleted', '0');
        $this->db->order_by('tbl_vocabulary_category.id', 'DESC');
    ...
    public function get_single_english_vocabulary_cat()
    {
        $this->db->where('tbl_vocabulary_category.is_deleted', '0');
        $this->db->where('tbl_vocabulary_category.status', '1');
        $this->db->where('tbl_vocabulary_category.id', $this->uri->segment(3));
        $result = $this->db->get('tbl_vocabulary_category');
        $result = $result->row();
        return $result;
    }
    ...
    public function delete_english_vocabulary_form($id)
    {
        $id = $this->uri->segment(3);
        $data = array(
            'new_status' => 0,
        ...
        $this->db->where('id', $id);
        $this->db->update('english_vocabulary', $data);
    }
    ...
    public function set_english_vocabulary_details($upload_image)
    {
        // Debugging input data (remove these in production)
        // echo "<pre>";
        // print_r($_POST);
        ...
        // Prepare data to insert/update
        $data = array(
            'english_vocabulary_image' => $upload_image,
            'category' => $this->input->post('category'),
            'english_vocabulary_title' => $this->input->post('title'),
            'english_vocabulary_description' => $this->input->post('description'),
        ...
        if ($id) {
            ...
            $this->db->from('english_vocabulary');
            $this->db->where('id', $id);
            $query = $this->db->get();
            ...
            if ($existingRecord) {
                ...
                if ($currentSequenceNo != $newSequenceNo) {
                    ...
                    if ($newSequenceNo < $currentSequenceNo) {
                        ...
                        $this->db->update('english_vocabulary');
                    } else {
            ...
            $this->db->insert('english_vocabulary', $data);
            return 1; // Indicating successful insert
        }
    }


    public function get_single_english_vocabulary()
    {
        $this->db->where('english_vocabulary.is_deleted', '0');
        // $this->db->where('english_vocabulary.status', '1');
        $this->db->where('english_vocabulary.new_status', '1');
        $this->db->where('english_vocabulary.id', $this->uri->segment(3));
