...
//Table data featching.
    ...
    function getStatistic() {
        ...
        $.ajax({
            ...
            var v1 = data["Users"];
            var v2 = data["admin_users"];
            var v3 = data["active_members"];
            var v4 = data["total_tests"];

            $('#Users').countTo({from: 0, to: v1, speed: 1000, interval: 20});
            $('#admin_Users').countTo({from: 0, to: v2, speed: 1000, interval: 20});
            $('#active_members').countTo({from: 0, to: v3, speed: 1000, interval: 20});
            $('#total_tests').countTo({from: 0, to: v4, speed: 1000, interval: 20});
        }).fail(function () {
            alert("Error.");
        });
    }
