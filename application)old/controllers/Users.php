...
class Users extends CI_Controller
{
    ...
    // echo '<pre>'; print_r($profile_pic); exit();

        if ($id != '') {
            $data = $this->input->post();
            if ($this->input->post('password') != '') {
                if ($this->input->post('currentpassword') != '') {
                    $old_row = $this->Users_model->getDataByid('panel_users', $this->input->post('users_id'), 'id');
            ...
            $this->Users_model->updateRow('panel_users', 'id', $id, $data);
    ...
    function delete_user(){
        $this->load->model("Users_model");
        $result = $this->Users_model->delete_user($this->uri->segment(2));
        $art_msg['msg'] = 'your data deleted successfully';
        $art_msg['type'] = 'success';
        $this->session->set_userdata('alert_msg', $art_msg);
    ...
    function fetch_user()
    {
        ...
        foreach ($fetch_data as $row) {
           ...
           <i class="material-icons">settings</i></button>
       		<a id="delete_' . $row->id . '" href="'.base_url().'delete_user/'.$row->id.'" class="btn bg-red waves-effect btn-xs">
    ...
    public function updateUser()
    {
        ...
        if (!$name || !$mobile || !$email || !$password || !$type || !$status) {
            ...
            $result = $this->Users_model->update($id, array("name" => $name, "mobile" => $mobile, "email" => $email, "password" => $password, "type" => $type, "status" => $status));
            redirect('users');
        }
    }
}
