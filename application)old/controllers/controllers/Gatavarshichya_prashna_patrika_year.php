...
class Gatavarshichya_prashna_patrika_year extends CI_Controller
{
    ...
    function index()
    {
        ...
        $this->load->view('gatavarshichya_prashna_patrika_year/index', $data);
        $this->load->view('gatavarshichya_prashna_patrika_year/add',$data);
        $this->load->view('gatavarshichya_prashna_patrika_year/edit',$data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('gatavarshichya_prashna_patrika_year/jscript.php', $data);
    ...
    function fetch_data()
    {
        ...
        $sql="SELECT year_id, question_paper_year, status, created_at, (SELECT gatavarshichya_prashna_patrika.exam_name AS question_paper_id FROM gatavarshichya_prashna_patrika WHERE gatavarshichya_prashna_patrika.gatavarshichya_prashna_patrika_id  = gatavarshichya_prashna_patrika_year.question_paper_id) AS question_paper_id FROM gatavarshichya_prashna_patrika_year ORDER BY year_id DESC";
    ...
    function add_data()
    {
        if(isset($_POST))
        {
            ...
            $sql="SELECT * FROM `gatavarshichya_prashna_patrika_year` WHERE question_paper_id='".$question_paper_id."' AND question_paper_year = '".$question_paper_year."' ";
            // echo $sql;
            $check=$this->common_model->executeRow($sql);
            if(!$check)
            {
                $sql="INSERT INTO `gatavarshichya_prashna_patrika_year`(`question_paper_id`, `question_paper_year`, `status`, `created_at`) VALUES ('".$question_paper_id."', '".$question_paper_year."', '".$status."', '".date('Y-m-d H:i:s')."')";
                $insert=$this->common_model->executeNonQuery($sql);
                if($insert)
                {
                    $art_msg['msg'] = 'New Gatavarshichya prashna patrika year added.';
            ...
            {
                $art_msg['msg'] = 'New gatavarshichya prashna patrika year already present.';
                $art_msg['type'] = 'error';
            }
        }
        $this->session->set_userdata('alert_msg', $art_msg);

        redirect(base_url() . 'Gatavarshichya_prashna_patrika_year', 'refresh');
    ...
    public function CategoryById($id="")
    {
        ...
        if($id!="")
        {
            $sql="SELECT * FROM gatavarshichya_prashna_patrika_year WHERE year_id ='".$id."' ";
            $check=$this->common_model->executeRow($sql);
            if($check)
            {
    ...
    public function update_data()
    {
        ...
        $sql="UPDATE gatavarshichya_prashna_patrika_year SET `question_paper_year`='".$edit_question_paper_year."', `question_paper_id`='".$edit_question_paper_id."', `status`='".$edit_status."' WHERE year_id ='".$edit_id."' ";
       
        $update=$this->common_model->executeNonQuery($sql);
        if($update)
        {
            $art_msg['msg'] = 'gatavarshichya prashna patrika year name Updated.';
            $art_msg['type'] = 'success';
            $this->session->set_userdata('alert_msg', $art_msg);
        }
        else
        {
            $art_msg['msg'] = 'Error to update gatavarshichya prashna patrika exam name.';
            $art_msg['type'] = 'error';
            $this->session->set_userdata('alert_msg', $art_msg);
        }
    ...
    function deleteCategory()
    {
        if($_REQUEST['id'])
        {
           ...
           $sql="SELECT * FROM gatavarshichya_prashna_patrika_year WHERE year_id='".$id."' ";
            ...
            if($check)
            {
                $sql="DELETE FROM gatavarshichya_prashna_patrika_year WHERE year_id='".$id."'";
