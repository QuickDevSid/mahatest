...
<script type="text/javascript">
    ...
    function getData() {
        ...
        var ur = "<?php echo base_url() ?>news/fetch_user";
        //Exportable table
        $('#user_data').DataTable({
            dom: 'Bfrtip',
            destroy: true,
    ...
    function getPostDetails(getID) {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo base_url() ?>news/postById_D/" + lid, // replace 'PHP-FILE.php with your php file
            dataType: "json",
            success: function(result) {
                //console.log(result);return false;
                $("#d_post_title").html(result[0]["news_title"]);
                ...
                if (result[0]["news_image"]) {
                    $("#d_post_image").html('<img class="img-fluid rounded" style="width: 100%;" src="<?php echo base_url() ?>AppAPI/current-affairs/' + result[0]["news_image"] + '" alt="">');
                }
                $("#d_post_description").html(result[0]["news_description"]);
    ...
    function getPostComment(getID) {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo base_url() ?>news/commentById/" + getID, // replace 'PHP-FILE.php with your php file
            dataType: "json",
            success: function(result) {
                let obj = result;
                let comment = "";
    ...
    function getPostDetailsEdit(getID) {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo base_url() ?>news/postById/" + lid, // replace 'PHP-FILE.php with your php file
            dataType: "json",
            success: function(result) {
                $('#edit_id').val(result[0]["current_affair_id"]);
                $('#post_image_old').val(result[0]["news_image"]);
                $('#edit_PostTitle').val(result[0]["news_title"]);
                ...
                CKEDITOR.instances['edit_Description'].setData(result[0]["news_description"]);
            },
            error: function() {
                alert('Some error occurred!');
            }
