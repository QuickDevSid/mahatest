...
//Table data featching.
        var ur = "<?php echo base_url() ?>Videos/fetch_data";
        //Exportable table
        $('#user_data').DataTable({
            dom: 'Bfrtip',
            destroy: true,
    ...
    function getDetailsEditYoutube(getID) {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo base_url() ?>Videos/CategoryById/" + lid, // replace 'PHP-FILE.php with your php file
            dataType: "json",
            success: function (result) {
                if(result["id"])
                {
                    ...
                    $('#edit_exam_y').selectpicker('val', result["selected_exams_id"]);
                    $('#edit_video_status_y').selectpicker('val', result["payment_status"]);
            ...
            success: function (result) {
                if(result["id"])
                {
                   ...
                   // $('#edit_URL_u').val(result["video_url"]);
                    ...
                    $('#edit_exam_u').selectpicker('val', result["selected_exams_id"]);
                    $('#edit_video_status_u').selectpicker('val', result["payment_status"]);
        ...
        $.ajax({
            ...
            success: function (result) {
                if(result["id"])
                {
                    ...
                    $('#edit_exam_v').selectpicker('val', result["selected_exams_id"]);
                    $('#edit_video_status_v').selectpicker('val', result["payment_status"]);
    ...
    function showConfirmMessage(getID) {
        ...
        if (lid === "") {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() ?>Videos/deleteCategory",
                data: "id=" + lid,
    ...
    $('#video_type').change(function(){
        ...
        if(type=='upload'){
            ...
            $('#vimeo_video_div').css('display','none');
            $('#youtube_video_div').css('display','none');
        }else if(type=='youtube'){
            $('#upload_video_div').css('display','none');
            $('#vimeo_video_div').css('display','none');
            $('#youtube_video_div').css('display','block');
        }else if(type=='vimeo'){
            $('#upload_video_div').css('display','none');
            $('#vimeo_video_div').css('display','block');
            $('#youtube_video_div').css('display','none');
        }else{
            $('upload_video_div').css('display','none');
            $('vimeo_video_div').css('display','none');
