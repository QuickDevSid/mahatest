...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#Sarav_prashnasanch_main').addClass('active');
        $('#Sarav_prashnasanch_main .menu-toggle').addClass('toggled');
        $('#Sarav_prashnasanch_main .ml-menu').css('display','block');

        $('#quiz_ans').prop('readonly', true);
      
        $('#Sarav_prasnasanch').addClass('active');
...
//Table data featching.
        var ur = "<?php echo base_url() ?>sarav_prasnasanch/fetch_data";
        //Exportable table
        $('#user_data').DataTable({
            dom: 'Bfrtip',
            destroy: true,
   ...
   $("#addquizform").submit(function (e) {
       ...
       $.ajax({
           type: "POST",
           url: "<?php echo base_url() ?>sarav_prasnasanch/addQuiz",
  ...
  function getquizDetails(getID) {
      ...
      $.ajax({
          type: "GET",
          url: "<?php echo base_url() ?>sarav_prasnasanch/QuizById/" + lid, // replace 'PHP-FILE.php with your php file
          dataType: "json",
          success: function (result) {
           // console.log(result);
              //alert(lid);
              $('#s_quiz_id').val(result["quiz_id"]);
              $("#s_quiz_title").val(result["quiz_title"]);
              $("#s_quiz_questions").val(result["quiz_questions"]);
              $("#s_quiz_duration").val(result["quiz_duration"]);
              $("#s_exams").val(result["selected_exams_id"]);
              $("#s_subject_name").val(result["sarav_prashnasanch_subjects_id"]);
      ...
      $.ajax({
          ...
          success: function (result) {
           // console.log(result);
              ...
              $("#e_quiz_title").val(result["quiz_title"]);
              $("#e_quiz_questions").val(result["quiz_questions"]);
              $("#e_quiz_duration").val(result["quiz_duration"]);

              
              // $('#e_exams').selectpicker('val', result["selected_exams_id"]);

                    for(i=0;i<result["selected_exams_id"].length;i++)
                    {
                        $('#e_exams').selectpicker('val', result["selected_exams_id"][i]);
                        get_select_value_edit(result["selected_exams_id"][i],result["sarav_prashnasanch_subjects_id"]);

//                        alert(result["sarav_prashnasanch_subjects_id"]);
                    }

               // $('#e_subject_name').selectpicker('val', result["sarav_prashnasanch_subjects_id"]);
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo base_url() ?>sarav_prasnasanch/Quizbyadd",
...
//code for delete user
    function showConfirmMessage_1(getID) {
          var lid = getID;

          swal({
              title: "Are you sure?",
              text: "Quiz will be deleted with ID : " + lid + "!",
              type: "warning",
      ...
      function deletequizDetailsPerform(getID) {
         ...
         // alert(lid);
          ...
          if (lid === "") {
              ...
              $.ajax({
                  type: "DELETE",
                  url: "<?php echo base_url() ?>sarav_prasnasanch/deletequiz/"+lid+"",
                  data: "id=" + lid,
                  success: function (data) {
                      $('#success_message').html(data);
                      console.log(data);
...
function addquizQuaDetails(getID) {
    var lid = getID.replace('Add_', '');
      $('#quiz_id').val(lid);
      var ur = "<?php echo base_url() ?>sarav_prasnasanch/fetch_qua_ans/" + lid;
      //Exportable table
    //  alert(lid);
      setTimeout(function() {
          //your datatable code
...
//alert(lid);
  ...
  function myFunctionSuc()
  {
    ...
    {
      $.post("<?php echo base_url()?>sarav_prasnasanch/get_qtn_details",{id:id},function(res){
        var str=$.trim(res);
        // alert(str);
      ...
      var explanation=CKEDITOR.instances["explanation"].getData();
              ...
              if(quiz_quation!="" && quiz_opt1!="" && quiz_opt2!="" && quiz_opt3!="" && quiz_opt4!="" && quiz_ans!="" && quiz_id!="")
                ...
                ({
                    ...
                    url: "<?php echo base_url() ?>sarav_prasnasanch/addquation",
                    type:"post",
                    ...
                    // cache:true,
                    // async:false,
                      data:{"quiz_id":quiz_id,"quiz_quation":quiz_quation, "quiz_opt1":quiz_opt1, "quiz_opt2":quiz_opt2, "quiz_opt3":quiz_opt3, "quiz_opt4":quiz_opt4, "quiz_ans":quiz_ans, "quiz_status":quiz_status, "subject_id":subject_id, "edit_id":edit_id, "type":type, "explanation":explanation},
       ...
       function deletequaDetailsPerform(getID,uid) {
          ...
          // alert(lid);
           ...
           if (lid === "") {
               ...
               $.ajax({
                   type: "POST",
                   url: "<?php echo base_url() ?>sarav_prasnasanch/deletequizqua/"+lid+"",
                   data: "id=" + lid,
                   success: function (data) {
                       $('#success_message').html(data);
                       console.log(data);
