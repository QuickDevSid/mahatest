...
class Gatavarshi_prashna_patrika extends CI_Controller
{
    ...
    function index()
    {
        ...
        $this->load->view('gatavarshi_prashna_patrika/index', $data);
        $this->load->view('gatavarshi_prashna_patrika/add',$data);
        $this->load->view('gatavarshi_prashna_patrika/edit',$data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('gatavarshi_prashna_patrika/jscript.php', $data);
    ...
    function fetch_data()
    {
        ...
        $sql="SELECT * FROM gatavarshichya_prashna_patrika ORDER BY gatavarshichya_prashna_patrika_id DESC";
    ...
    function add_data()
    {
        if(isset($_POST))
        {
            ...
            $sql="SELECT * FROM `gatavarshichya_prashna_patrika` WHERE selected_exam_id='".$exam_Id."' AND exam_name = '".$Title."' ";
            // echo $sql;
            $check=$this->common_model->executeRow($sql);
            if(!$check)
            {
                $sql="INSERT INTO `gatavarshichya_prashna_patrika`(`selected_exam_id`, `exam_name`, `status`, `created_at`) VALUES ('".$exam_Id."', '".$Title."', '".$status."', '".date('Y-m-d H:i:s')."')";
                $insert=$this->common_model->executeNonQuery($sql);
                if($insert)
                {
                    $art_msg['msg'] = 'New Gatavarshichya prashna patrika exam name updated.';
            ...
            {
                $art_msg['msg'] = 'New gatavarshichya prashna patrika exam already present.';
                $art_msg['type'] = 'error';
            }
        }
        $this->session->set_userdata('alert_msg', $art_msg);

        redirect(base_url() . 'Gatavarshi_prashna_patrika', 'refresh');
    ...
    public function CategoryById($id="")
    {
        ...
        if($id!="")
        {
            $sql="SELECT * FROM gatavarshichya_prashna_patrika WHERE gatavarshichya_prashna_patrika_id='".$id."' ";
            $check=$this->common_model->executeRow($sql);
            if($check)
            {
    ...
    public function update_data()
    {
        ...
        $sql="UPDATE gatavarshichya_prashna_patrika SET `selected_exam_id`='".$exam_id."', `exam_name`='".$Title."', `status`='".$status."' WHERE gatavarshichya_prashna_patrika_id='".$edit_id."' ";
        // echo $sql;
        // exit(0);
    ...
    function deleteCategory()
    {
        if($_REQUEST['id'])
        {
           ...
           $sql="SELECT * FROM gatavarshichya_prashna_patrika WHERE gatavarshichya_prashna_patrika_id='".$id."' ";
            ...
            if($check)
            {
                $sql="DELETE FROM gatavarshichya_prashna_patrika WHERE gatavarshichya_prashna_patrika_id='".$id."'";
