...
class Gatavarshichya_prashna_patrika_practice_test extends CI_Controller
{
    ...
    function index()
    {
        $data['title'] = ucfirst('Tests'); // Capitalize the first letter
        $this->load->view('templates/header1', $data);
        $this->load->view('templates/menu', $data)
        ;
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/index', $data);
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/add',$data);
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/add_quize_qua',$data);
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/show_quize',$data);
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/edit',$data);
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/import_quiz_question',$data);
        $this->load->view('templates/footer1', $data);
        $this->load->view('gatavarshichya_prashna_patrika_practice_test/jscript.php', $data);
    }

    function fetch_data()
   {
       $sql="SELECT * FROM gatavarshichya_prashna_patrika_year_title_practice ORDER BY question_papers_id DESC";
       $fetch_data=$this->common_model->executeArray($sql);
       $data = array();
       foreach ($fetch_data as $row) {
           $sub_array = array();
    
           $sql="SELECT * FROM practice_test_year WHERE year_id='".$row->question_paper_year."' ";
           // echo $sql;
           $year=$this->common_model->executeRow($sql);
           
            $sql1="SELECT * FROM gatavarshiche_prashna_subjects WHERE gatavarshichya_prashna_patrika_id='".$year->question_paper_id."' ";
           // echo $sql;
           $exam=$this->common_model->executeRow($sql1);
   ...
   public function QuizById($id=0)
   {
        ...
        if($id!="")
        {
            $sql="SELECT * FROM gatavarshichya_prashna_patrika_year_title_practice WHERE question_papers_id='".$id."' ";
          //  echo $sql;
            $check=$this->common_model->executeRow($sql);
            if($check)
            {
                $sql="SELECT * FROM practice_test_year WHERE year_id='".$check->question_paper_year."' ";
                // echo $sql;
                $year=$this->common_model->executeRow($sql);
   ...
   {
      $id      = $cid;
      $sql="SELECT * FROM gatavarshichya_prashna_patrika_practice_test WHERE question_papers_id='".$id."' ";
       $fetch_data = $this->common_model->executeArray($sql);
       $data = array();
       foreach ($fetch_data as $row) {
          ...
          <button type="button" name="Delete" onclick="deletequaDetails(this.id,this.value)" value="'.$row->practice_test_id.'"  id="delete_' . $row->practice_test_id . '" data-type="confirm" class="btn bg-red waves-effect btn-xs">
          <i class="material-icons">delete</i></button>

          <button type="button" name="edit" onclick="edit_qtn('.$row->practice_test_id.')"  data-type="confirm" class="btn bg-red waves-effect btn-xs">
   ...
   public function update_exam()
   {
        ...
        if($check)
        {
            ...
            if($update)
            {
              ...
              //  $this->common_model->executeNonQuery($sql);
                ...
                $art_msg['msg'] = 'Test Updated.';
                $art_msg['type'] = 'success';
                $this->session->set_userdata('alert_msg', $art_msg);
            }
            else
            {
                $art_msg['msg'] = 'Error to update Test.';
                $art_msg['type'] = 'error';
                $this->session->set_userdata('alert_msg', $art_msg);
            }
        }
        ...
        {
            $art_msg['msg'] = 'error occurred.';
            $art_msg['type'] = 'error';
            $this->session->set_userdata('alert_msg', $art_msg);
        }
        redirect(base_url() . 'Gatavarshichya_prashna_patrika_practice_test', 'refresh');
   ...
   public function add_exam()
   {
        ...
        $sql="SELECT * FROM gatavarshichya_prashna_patrika_year_title_practice WHERE paper_title='".$a_quiz_title."' ";
        $check=$this->common_model->executeRow($sql);
        if($check)
        {
            $art_msg['msg'] = 'Gatavarshiche Test already present.';
            $art_msg['type'] = 'error';
            $this->session->set_userdata('alert_msg', $art_msg);
        }
        else
        {
            $sql="INSERT INTO `gatavarshichya_prashna_patrika_year_title_practice`( `paper_title`, `total_questions`, `duration`, `correct_answer_mark`, `wrong_answer_mark`, `instructions`, `status`, `created_at`, `pdf_url`, `question_paper_year`) VALUES ('".$a_quiz_title."', '".$a_quiz_questions."', '".$a_quiz_duration."', '".$a_correct_answer_mark."', '".$a_wrong_answer_mark."', '".$a_instructions."', '".$a_status."', '".date('Y-m-d H:i:s')."', '".$pdf."', '".$a_year."')";
            $update=$this->common_model->executeNonQuery($sql);
            if($update)
            {
              ...
              //  $this->common_model->executeNonQuery($sql);
                ...
                $art_msg['msg'] = 'New Gatavarshiche Exam Added.';
                $art_msg['type'] = 'success';
                $this->session->set_userdata('alert_msg', $art_msg);
            }
            else
            {
                $art_msg['msg'] = 'Error to Adding Gatavarshiche Test.';
                $art_msg['type'] = 'error';
                $this->session->set_userdata('alert_msg', $art_msg);
            }
        }
   ...
   public function addquation()
   {
    ...
    // print_r($_POST);
     ...
     if($edit_id==0)
     {
         $sql="SELECT * FROM gatavarshichya_prashna_patrika_practice_test WHERE question_papers_id='".$quiz_id."' AND question='".$quiz_quation."' ";
         $check=$this->common_model->executeRow($sql);
         if(!$check)
         {
          $sql="INSERT INTO `gatavarshichya_prashna_patrika_practice_test`( `question_papers_id`,`subject_id` ,`question`, `option1`, `option2`, `option3`, `option4`, `answer`, `explanation`, `status` ) VALUES ('".$quiz_id."','".$subject_id."', '".$quiz_quation."', '".$quiz_opt1."', '".$quiz_opt2."', '".$quiz_opt3."', '".$quiz_opt4."', '".$quiz_ans."', '".$explanation."' ,'".$quiz_status."' )";
            // exit(0);
      ...
      {
        $sql="UPDATE gatavarshichya_prashna_patrika_practice_test SET `question`='".$quiz_quation."', `option1`='".$quiz_opt1."', `option2`='".$quiz_opt2."', `option3`='".$quiz_opt3."', `option4`='".$quiz_opt4."', `answer`='".$quiz_ans."', `explanation`='".$explanation."', `status`='".$quiz_status."', `subject_id`='".$subject_id."' WHERE practice_test_id='".$edit_id."' ";
    ...
    function get_qtn_details()
    {
      ...
      if(isset($_POST['id']))
      {
        $id=$_POST['id'];
         ...
         $sql="SELECT * FROM gatavarshichya_prashna_patrika_practice_test WHERE practice_test_id='".$id."' ";
         // echo $sql;
         $check=$this->common_model->executeRow($sql);
         if($check)
         {
   ...
   public function deletequizqua($id)
   {
        ...
        if($check)
        {
            $sql="DELETE FROM gatavarshichya_prashna_patrika_practice_test WHERE practice_test_id='".$id."'";
    ...
    public function deletequiz($id)
    {
        ...
        if($check)
        {
            $sql="DELETE FROM gatavarshichya_prashna_patrika_year_title_practice WHERE question_papers_id='".$id."'";
            $delete=$this->common_model->executeNonQuery($sql);
            if($delete)
            {
                echo "Success";
                
                $sql="DELETE FROM gatavarshichya_prashna_patrika_practice_test WHERE question_papers_id='".$id."'";
                $this->common_model->executeNonQuery($sql);
            }
            else
            {
    ...
    public function get_select_1()
    {
        ...
        if(isset($_POST['id']))
        {
            ...
            $sql="SELECT * FROM `practice_test_year` WHERE question_paper_id='".$id."' ";
            $check=$this->common_model->executeArray($sql);
            if($check)
            {
                foreach ($check as $value)
    ...
    public function importQuestions(){
        $this->load->model("Gatavarshichya_prashna_patrika_practice_test_model");
        ...
        if(empty($error)){
            ...
            try {
                ...
                $result = $this->Gatavarshichya_prashna_patrika_practice_test_model->insert($inserdata);
                if($result){
