...
if ( isset( $_POST['quiz_result_id'] ) ) {
    ...
    $query = "SELECT * FROM 
                    previous_exam_live_test_result_details a, previous_exam_live_test_result b, gatavarshichya_prashna_patrika_live_test c 
                    WHERE a.previous_exam_live_test_result_id = b.previous_exam_live_test_result_id 
                    AND a.question = c.question 
                    AND a.previous_exam_live_test_result_id = $quiz_result_id
                    GROUP BY a.previous_exam_live_test_result_details_id";

    $statement = $connect->query( $query );
    $result    = $statement->fetchAll( PDO::FETCH_ASSOC );

//Get questions details for pagination.
    $id_query = "SELECT question_papers_id FROM previous_exam_live_test_result WHERE previous_exam_live_test_result_id = '$quiz_result_id'";
    ...
    FROM gatavarshichya_prashna_patrika_live_test r, quiz_subject s WHERE r.question_papers_id = '$quiz_id' 
    ...
    foreach ($result1 as $row) {
        $sid = $row['SubjectID'];
        $query_question = "SELECT live_test_id as id, question FROM gatavarshichya_prashna_patrika_live_test 
