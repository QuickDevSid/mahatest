...
if ( isset( $_POST['question_papers_id'] ) ) {
    ...
    $query = "SELECT * FROM gatavarshichya_prashna_patrika_live_test WHERE `question_papers_id` = '$question_papers_id' AND status = 'Active'  ORDER BY gatavarshichya_prashna_patrika_live_test.live_test_id  ";
    ...
    $result    = $statement->fetchAll( PDO::FETCH_ASSOC );
     ...
     FROM gatavarshichya_prashna_patrika_live_test r, quiz_subject s WHERE r.question_papers_id = '$question_papers_id'
     AND r.subject_id = s.subject_id AND r.status = 'Active' GROUP BY s.subject_id ORDER BY r.live_test_id ";
