...
if (isset($_POST['quiz_id'])) {
    ...
    //Get question details for exams
    $query = "SELECT * FROM daily_quiz_questions WHERE `quiz_id` = '$quiz_id' AND status = 'Active' ORDER BY daily_quiz_questions.daily_quiz_questions_id ";
    ...
    FROM daily_quiz_questions r, quiz_subject s WHERE r.quiz_id = '$quiz_id'
    AND r.subject_id = s.subject_id AND r.status = 'Active' GROUP BY s.subject_id ORDER BY r.daily_quiz_questions_id";
    ...
    foreach ($result1 as $row) {
        $sid = $row['SubjectID'];
        $query_question = "SELECT daily_quiz_questions_id as id, question FROM daily_quiz_questions
        WHERE quiz_id = '$quiz_id' AND subject_id = '$sid' AND status = 'Active'";
